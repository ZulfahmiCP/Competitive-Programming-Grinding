...
struct DIGIT_DP {
    ...
    ll answer(int n, int a, int b, bool k, bool t) {
        if(n == sz(digits)) return (k ? 0 : (a == 0 || b == 0));
        if(dp[n][a][b][k][t] != -1) return dp[n][a][b][k][t];
        ...
        for(int d = 0; d <= m; d++){
            if(k && d == 0){
                ...
                if(ok) 
                    res += answer(n + 1, (a + d) % 3, (b * 10 + d) % 5, 0, t && (d == m));
            }
        }
        
        return dp[n][a][b][k][t] = res;
    }
