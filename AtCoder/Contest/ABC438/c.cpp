...
// int order_of_key(ll v) {return S.order_of_key(v);}
...
int main() {
    ...
    for(int i = 0; i <= sz(ms) - 4; i++){
        vector<int> arr;

        auto it = ms.find_by_order(i);

        while(it != ms.end() && sz(arr) < 4){
            arr.pb(*it);
            it++;
        }
        ...
        for(int j = 0; j < 4; j++){
            int k = ms.order_of_key(arr[j]);
            auto x = ms.find_by_order(k);
            ms.erase(x);
        }
