...
struct RabinKarp {
    vector<int> find(const string& s, const string& p, int base = 25) {
        int n = sz(s), m = sz(p);
        vector<mint> S(n + 1), power(max(n, m));
        vector<int> pos;
        ...
        power[0] = 1;
        for(int i = 1; i < sz(power); i++)
            power[i] = power[i - 1] * base;
        
        for(int i = 0; i < n; i++)
            S[i + 1] = S[i] + (s[i] - 'a' + 1) * power[i];
        
        mint hash;
        for(int i = 0; i < m; i++)
            hash += (p[i] - 'a' + 1) * power[i];
        
        for(int i = 0; i <= n - m; i++)
            if(S[i + m] - S[i] == hash * power[i])
                pos.pb(i);
