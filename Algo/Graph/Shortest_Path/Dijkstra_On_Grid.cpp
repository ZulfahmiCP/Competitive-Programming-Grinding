...
struct Grid {
    ...
    Grid(int n, int m) : N(n), M(m), maze(N, vector<int>(M)), vis(N, vector<bool>(M, 0)),
                         dist(N, vector<int>(M, INFL)), parent(N, vector<pair<int, int>>(M)) {}

    int find_path(int sr, int sc, int tr, int tc) {
        min_heap<pair<ll, pair<int, int>>> pq;
        pq.push({0, {sr, sc}});
        dist[sr][sc] = 0;
        parent[sr][sc] = {-1, -1};
        ...
        return dist[tr][tc] == INFL ? -1 : dist[tr][tc];
    }
