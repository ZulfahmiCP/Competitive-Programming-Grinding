...
int main() {
    ...
    for(int i = 0; i < n; i++)
        seg.update(A[i], A[i], (-1) + (i - 2 * B[i]));

    // debug(inv);
    // debug(B);

    for(int i = 0; i < n; i++){
        ans = min({ans, inv + seg.calc_min(0, A[i]-1), inv + seg.calc_min(A[i] + 1, n - 1)});
        seg.update(0, A[i] - 1, 1);
        seg.update(A[i] + 1, n - 1, -1);
        seg.update(A[i], A[i], -seg.calc_sum(A[i], A[i]) + INFL);
    }
