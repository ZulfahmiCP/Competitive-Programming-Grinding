...
struct Tree {
    ...
    vector<map<int, ll>> cnt_mp, dist_mp;
    vector<int> sub, vis, par_cen, dep;
    vector<ll> cnt, dist;
    vector<bool> exist;
    ll ans = 0;

    Tree(int n) : N(n), K(log2(N) + 1), adj(N), sub(N), vis(N, 0), cnt(N), dist(N), par_cen(N),
    par(N, vector<int>(K, -1)), dep(N, 0), exist(N, 0), cnt_mp(N), dist_mp(N) {}
    ...
    void add(int u) {
        ...
        for(int v = par_cen[u], c = u; v != -1; c = v, v = par_cen[v]){
            ans += dist[v] - dist_mp[v][c];
            ans += (cnt[v] - cnt_mp[v][c]) * distance(u, v);
        }
