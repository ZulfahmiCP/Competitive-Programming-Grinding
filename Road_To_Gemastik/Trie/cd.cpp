...
struct Trie {
    ...
    void dfs(int x) {
        ...
        sort(all(next), [&](pair<int, char> &a, pair<int, char> &b){
            return trie[a.fi].dp < trie[b.fi].dp;
        });
        // debug(next);
...
int main(){
    ...
    trie.dfs(0);
    while(trie.ans.back() == '-') trie.ans.ppb();
    cout << sz(trie.ans) << endl;
    for(char c : trie.ans) cout << c << endl;
