    ...
    void insert(const string &s, int j) {
        ...
        for(char c : s) {
            ...
            if(y > trie[x].mx){
                trie[x].mx = y;
                trie[x].id = j;
            } else if(y == trie[x].mx && s < S[trie[x].id]) {
                trie[x].id = j;
            }
        }
    }

    pair<int, int> max_occ(const string &s) {
        int x = 0;
...
int main(){
    ...
    for(int i = 0; i < q; i++){
        cin >> s;
        auto [j, c] = trie.max_occ(s);
        
        if(j != -1)
            cout << trie.S[j] << ' ' << c << endl;
        else 
            cout << "-1\n"; 
    }
