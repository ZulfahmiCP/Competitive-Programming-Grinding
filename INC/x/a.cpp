...
struct Graph {
    ...
    Graph(int n, int m) : N(n), M(m), adj(N + M), vis(N + M, 0), mate(N + M, -1) {}
...
int main() {
    ...
    int n,m(0),k; cin >> n >> k;
    vector<vector<array<int, 3>>> A(k);

    for(int i = 0, j; i < k; i++){
        cin >> j;
        A[i] = vector<array<int, 3>>(j);
    ...
    for(int i = 0, cur = n; i < k; i++){
        for(int j = 0; j < sz(A[i]); j++){
            graph.add_edge(A[i][j][0], cur);
            graph.add_edge(A[i][j][1], cur);
            graph.add_edge(A[i][j][2], cur);
            cur++;
        }
    }
